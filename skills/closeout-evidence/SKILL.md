...
3. Reconcile `functionalStatus` and `usabilityStatus` independently, then reconcile aggregate `PASS`, `FAIL`, `REVIEW`, `BLOCKED`, and `SKIP` without converting missing evidence into success.
...
7. Confirm every P0-P3 finding has at least one referenced evidence artifact.
8. Recommend the smallest follow-up workflow justified by unresolved findings.
9. State coverage limits and excluded surfaces plainly.
